...
var babelPluginTransformVueJsx = _interopDefault(require("@vue/babel-plugin-transform-vue-jsx")),
  ...
  index = (a, {
    ...
    if (f) {
      if (["native", "naruto"].includes(f) && (h = "vue"), "vue-demi" === f && (h = "vue-demi"), ["auto", !0].includes(f)) try {
        const a = require("vue/package.json").version;
        a.startsWith("2.7") && (h = "vue");
      } catch (a) {}
      "object" == typeof f && f.importSource && (h = f.importSource), g = [babelSugarCompositionApiInjectH, {
        importSource: h
      }];
    }
    return {
      plugins: [b && babelSugarFunctionalVue, c && g, d && babelSugarVModel, e && babelSugarVOn, f && [babelSugarCompositionApiRenderInstance, {
        importSource: h
      }], babelPluginTransformVueJsx].filter(Boolean)
    };
  };
