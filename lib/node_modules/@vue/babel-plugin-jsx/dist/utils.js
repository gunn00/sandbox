...
const transformJSXMemberExpression = path => {
  ...
  const transformedObject = t.isJSXMemberExpression(objectPath) ? (0, exports.transformJSXMemberExpression)(path.get('object')) : t.isJSXIdentifier(objectPath) ? t.identifier(objectPath.name) : t.nullLiteral();
...
const getTag = (path, state) => {
  ...
  if (namePath.isJSXIdentifier()) {
    ...
    if (!html_tags_1.default.includes(name) && !svg_tags_1.default.includes(name)) {
      return name === exports.FRAGMENT ? (0, exports.createIdentifier)(state, exports.FRAGMENT) : path.scope.hasBinding(name) ? t.identifier(name) : ((_b = (_a = state.opts).isCustomElement) === null || _b === void 0 ? void 0 : _b.call(_a, name)) ? t.stringLiteral(name) : t.callExpression((0, exports.createIdentifier)(state, 'resolveComponent'), [t.stringLiteral(name)]);
    }
