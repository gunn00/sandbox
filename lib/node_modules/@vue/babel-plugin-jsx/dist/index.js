...
exports.default = ({
  ...
  visitor: Object.assign(Object.assign(Object.assign({}, transform_vue_jsx_1.default), sugar_fragment_1.default), {
    Program: {
      enter(path, state) {
        if (hasJSX(path)) {
          const importNames = ['createVNode', 'Fragment', 'resolveComponent', 'withDirectives', 'vShow', 'vModelSelect', 'vModelText', 'vModelCheckbox', 'vModelRadio', 'vModelText', 'vModelDynamic', 'resolveDirective', 'mergeProps', 'createTextVNode', 'isVNode'];
      ...
      exit(path) {
        ...
        body.filter(nodePath => t.isImportDeclaration(nodePath.node) && nodePath.node.source.value === 'vue').forEach(nodePath => {
          const {
            specifiers
          } = nodePath.node;
