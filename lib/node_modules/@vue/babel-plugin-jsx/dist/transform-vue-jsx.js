  ...
  props.forEach(prop => {
    if (prop.isJSXAttribute()) {
      ...
      if (!(0, utils_1.isConstant)(attributeValue) || name === 'ref') {
        if (!isComponent && (0, utils_1.isOn)(name)
        ...
        } else if (name !== 'key' && !(0, utils_1.isDirective)(name) && name !== 'on') {
          dynamicPropNames.add(name);
        }
      }
      if (state.opts.transformOn && (name === 'on' || name === 'nativeOn')) {
        if (!state.get('transformOn')) {
          state.set('transformOn', (0, helper_module_imports_1.addDefault)(path, '@vue/babel-helper-vue-transform-on', {
          ...
          values.forEach((value, index) => {
            ...
            if (!directive) {
              ...
              if ((_b = modifiers[index]) === null || _b === void 0 ? void 0 : _b.size) {
                properties.push(t.objectProperty(isDynamic ? t.binaryExpression('+', propName, t.stringLiteral('Modifiers')) : t.stringLiteral(`${((_c = propName) === null || _c === void 0 ? void 0 : _c.value) || 'model'}Modifiers`), t.objectExpression([...modifiers[index]].map(modifier => t.objectProperty(t.stringLiteral(modifier), t.booleanLiteral(true)))), isDynamic));
              }
            }
            const updateName = isDynamic ? t.binaryExpression('+', t.stringLiteral('onUpdate'), propName) : t.stringLiteral(`onUpdate:${((_d = propName) === null || _d === void 0 ? void 0 : _d.value) || 'modelValue'}`);
...
}).filter(value => value !== undefined && value !== null && !t.isJSXEmptyExpression(value));
  ...
  if (children.length > 1 || slots) {
    /*
      <A v-slots={slots}>{a}{b}</A>
        ...
        ---> {[a, b]}
    */
    VNodeChild = isComponent ? children.length ? t.objectExpression([!!children.length && t.objectProperty(t.identifier('default'), t.arrowFunctionExpression([], t.arrayExpression((0, utils_1.buildIIFE)(path, children)))), ...(slots ? t.isObjectExpression(slots) ? slots.properties : [t.spreadElement(slots)] : []), optimize && t.objectProperty(t.identifier('_'), t.numericLiteral(slotFlag))].filter(Boolean)) : slots : t.arrayExpression(children);
    ...
    const objectExpression = t.objectExpression([t.objectProperty(t.identifier('default'), t.arrowFunctionExpression([], t.arrayExpression((0, utils_1.buildIIFE)(path, [child])))), optimize && t.objectProperty(t.identifier('_'), t.numericLiteral(slotFlag))].filter(Boolean));
    if (t.isIdentifier(child) && isComponent) {
      ...
      if (enableObjectSlots) {
        ...
        const alternate = t.objectExpression([t.objectProperty(t.identifier('default'), t.arrowFunctionExpression([], t.arrayExpression((0, utils_1.buildIIFE)(path, [slotId])))), optimize && t.objectProperty(t.identifier('_'), t.numericLiteral(slotFlag))].filter(Boolean));
      ...
      VNodeChild = t.objectExpression([t.objectProperty(t.identifier('default'), child)]);
    } else if (t.isObjectExpression(child)) {
      VNodeChild = t.objectExpression([...child.properties, optimize && t.objectProperty(t.identifier('_'), t.numericLiteral(slotFlag))].filter(Boolean));
    } else {
      VNodeChild = isComponent ? t.objectExpression([t.objectProperty(t.identifier('default'), t.arrowFunctionExpression([], t.arrayExpression([child])))]) : t.arrayExpression([child]);
    }
  }
  const createVNode = t.callExpression((0, utils_1.createIdentifier)(state, 'createVNode'), [tag, props, VNodeChild || t.nullLiteral(), !!patchFlag && optimize && t.numericLiteral(patchFlag), !!dynamicPropNames.size && optimize && t.arrayExpression([...dynamicPropNames.keys()].map(name => t.stringLiteral(name)))].filter(Boolean));
  if (!directives.length) {
    return createVNode;
  }
  return t.callExpression((0, utils_1.createIdentifier)(state, 'withDirectives'), [createVNode, t.arrayExpression(directives)]);
};
exports.default = {
  JSXElement: {
    exit(path, state) {
