...
const CATEGORIES = ['essential', 'strongly-recommended', 'recommended', 'uncategorized'];
...
function getDefaultValue(rule, data) {
  ...
  return CATEGORIES.indexOf(ruleCategory) <= CATEGORIES.indexOf(currentCategory.split('/')[1]) ? RULE_SETTING_ERROR : RULE_SETTING_OFF;
}
function getEslintPrompts(data, rules) {
  ...
  return CATEGORIES.map(category => allRules.filter(rule => rule.meta.docs.category === category || category === 'uncategorized' && rule.meta.docs.category === undefined)).reduce((acc, rulesArr) => [...acc, ...rulesArr], []).map(rule => {
    const value = data.eslint && data.eslint.rules && data.eslint.rules[rule.name];
    return {
      name: rule.name,
      type: 'list',
      message: rule.name,
      ...
      choices: !value || RULE_SETTINGS.indexOf(value) > -1 ? defaultChoices : [...defaultChoices, {
