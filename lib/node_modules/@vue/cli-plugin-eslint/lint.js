...
const renamedObjectArgs = {
  env: {
    key: ['overrideConfig', 'env'],
    def: true
  },
  global: {
    key: ['overrideConfig', 'globals'],
    def: false
  }
};
const renamedArgs = {
...
module.exports = async function lint(args = {}, api) {
  ...
  const {
    ESLint
  } = loadModule('eslint', cwd, true) || require('eslint');
  ...
  // File enumeration
  'cwd', 'errorOnUnmatchedPattern', 'extensions', 'globInputPaths', 'ignore', 'ignorePath',
  // Linting
  'allowInlineConfig', 'baseConfig', 'overrideConfig', 'overrideConfigFile', 'plugins', 'reportUnusedDisableDirectives', 'resolvePluginsRelativeTo', 'rulePaths', 'useEslintrc',
  // Autofix
  'fix', 'fixTypes',
  // Cache-related
  'cache', 'cacheLocation', 'cacheStrategy'].map(k => [k, config[k]])));
  const defaultFilesToLint = [];
  for (const pattern of ['src', 'tests',
  // root config files
  '*.js', '.*.js']) {
    if ((await Promise.all(globby.sync(pattern, {
      cwd,
      absolute: true
    }).map(p => eslint.isPathIgnored(p)))).some(r => !r)) {
