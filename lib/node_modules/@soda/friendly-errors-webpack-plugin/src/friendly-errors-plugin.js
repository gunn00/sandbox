...
const defaultTransformers = [require('./transformers/babelSyntax'), require('./transformers/moduleNotFound'), require('./transformers/esLintError')];
const defaultFormatters = [require('./formatters/moduleNotFound'), require('./formatters/eslintError'), require('./formatters/defaultError')];
class FriendlyErrorsWebpackPlugin {
  ...
  displayErrors(errors, severity) {
    ...
    const subtitle = severity === 'error' ? `Failed to compile with ${nbErrors} ${severity}${nbErrors === 1 ? '' : 's'}` : `Compiled with ${nbErrors} ${severity}${nbErrors === 1 ? '' : 's'}`;
...
function extractErrorsFromStats(stats, type) {
  if (isMultiStats(stats)) {
    const errors = stats.stats.reduce((errors, stats) => errors.concat(extractErrorsFromStats(stats, type)), []);
