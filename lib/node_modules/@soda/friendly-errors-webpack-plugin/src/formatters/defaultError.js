...
function displayError(severity, error) {
  ...
  return concat(`${baseError} ${removeLoaders(error.file)}`, '', error.message, error.origin ? error.origin : undefined, '', error.infos);
}
...
function format(errors, type) {
  return errors.filter(isDefaultError).reduce((accum, error) => accum.concat(displayError(type, error)), []);
}
